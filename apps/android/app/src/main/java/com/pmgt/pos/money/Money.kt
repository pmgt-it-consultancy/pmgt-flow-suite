...
data class ItemTotals(
    val grossAmount: Double, val vatableAmount: Double, val vatAmount: Double,
    val vatExemptAmount: Double, val nonVatAmount: Double, val discountAmount: Double, val netAmount: Double,
)

data class OrderTotals(
    val grossSales: Double = 0.0, val vatableSales: Double = 0.0, val vatAmount: Double = 0.0,
    val vatExemptSales: Double = 0.0, val nonVatSales: Double = 0.0, val discountAmount: Double = 0.0,
    val netSales: Double = 0.0,
)

data class VatBreakdown(val vatExclusive: Double, val vatAmount: Double)
data class ScPwdDiscount(val discountedPrice: Double, val discountAmount: Double, val vatExemptAmount: Double)
...
object Money {
    ...
    fun normalizeVatRate(rate: Double): Double = if (rate <= 0.0) 0.0 else if (rate > 1.0) rate / 100.0 else rate
    ...
    fun itemTotals(unitPrice: Double, quantity: Double, isVatable: Boolean, scPwdQuantity: Double = 0.0, vatRate: Double = 0.12): ItemTotals {
        val rate = normalizeVatRate(vatRate)
        val gross = round(unitPrice * quantity)
        val regularGross = round(unitPrice * (quantity - scPwdQuantity))
        val vatable = isVatable && rate > 0.0
        val regular = if (vatable) vatBreakdown(regularGross, true, rate) else VatBreakdown(regularGross, 0.0)
        var discount = 0.0
        var exempt = 0.0
        ...
        return ItemTotals(gross, if (vatable) regular.vatExclusive else 0.0,
            if (vatable) regular.vatAmount else 0.0, exempt, if (!vatable) regularGross else 0.0,
            discount, round(round(regularGross) + discountedNet))
    }

    fun aggregate(items: List<ItemTotals>): OrderTotals = items.fold(OrderTotals()) { total, item ->
        OrderTotals(round(total.grossSales + item.grossAmount), round(total.vatableSales + item.vatableAmount),
            round(total.vatAmount + item.vatAmount), round(total.vatExemptSales + item.vatExemptAmount),
            round(total.nonVatSales + item.nonVatAmount), round(total.discountAmount + item.discountAmount),
    ...
    fun checkout(items: List<ItemTotals>, globalDiscount: Double = 0.0): OrderTotals {
        ...
        return if (globalDiscount > 0.0) totals.copy(discountAmount = totals.discountAmount + globalDiscount,
            netSales = max(0.0, totals.netSales - globalDiscount)) else totals
    }
