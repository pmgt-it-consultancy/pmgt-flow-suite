...
- The original pull `since`, server timestamp and pagination cursor remain pinned across pages. Every page yields to a macrotask before continuing.
...
- Final review found that a legitimate backlog beyond the former 50-page limit would otherwise replay its first 50 pages forever. That total-page cutoff is replaced with a bounded recent-cursor guard: a progressing run continues with its cursor in memory until complete, while an incomplete response repeating one of the last 50 canonical cursor maps fails into retry backoff. Only completion advances the durable watermark and publishes successful state. A write queued near page 50 remains pending and is sent in the follow-up run. The guard bounds retained cursor history and catches stuck cursors and short cycles; it cannot prove completion for a faulty server inventing indefinitely new cursors or cycles longer than its history window. Such a run still yields between pages and responds to stop/offline. No cursor identifiers are added to diagnostics or error text.
